...
export default function Header() {
  ...
  return (
    <header
      className="fixed inset-x-0 top-0 z-[9999] bg-blue-950 shadow backdrop-blur"
    >
          ...
          {navItems.map((item) =>
            item.dropdown ? (
              <div key={item.name} className="relative group">
                <button className="group relative flex items-center gap-1 rounded-md px-3 py-2  text-sm font-medium text-white">
                  {item.name}
                ...
                <div className="invisible absolute left-0 z-50 mt-2 md:mt-0 w-48 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 group-hover:visible">
